...
import { ECOSENSE_CARDS } from "@/utilities/constants";
...
export default function Iniciatives() {
  ...
  return (
    <div className="flex flex-col w-full text-center items-center gap-6">
      <h5 className="w-[70%] text-(--spg-text) hidden md:block leading-tight">{t("reducingImpact")}</h5>
      <h5 className="px-[5%] text-center text-(--spg-text) md:hidden leading-tight">{t("reducingImpactMovile")}</h5>
      <p className="px-[5%] md:w-[50%] text-sm md:text-md text-(--spg-text) leading-normal">{t("description")}</p>
      <div className="flex flex-col md:flex-row w-full mt-10">
        {ECOSENSE_CARDS.slice(0, 2).map((slide, idx) => (
          <InfoHighlightCard
            key={idx}
            imageSrc={slide.src}
